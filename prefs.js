...
class MyPreferencesWidget extends  GObject.Object {
...
export default class MyPreferences extends ExtensionPreferences {
    ...
    ['mouse-sensitive',
     ...
     'keep-round-corners'
        ...
        let settings_onoff = builder.get_object("toggle_" + s.replace(/-/g, "_"));
        settings_onoff.set_active(window._settings.get_boolean(s));
        settings_onoff.connect('notify::active', function (w) {
            window._settings.set_boolean(s, w.active);
        });
        window._settings.connect('changed::' + s, function (k,b) {
            settings_onoff.set_active(settings.get_boolean(b));
        });
    });
    ...
    ['pressure-threshold',
     'pressure-timeout'
        ...
        let settings_spin = builder.get_object("spin_" + s.replace(/-/g, "_"));
        settings_spin.set_value(window._settings.get_int(s));
        settings_spin.connect('value-changed', function (w) {
            window._settings.set_int(s, w.get_value());
        });
        window._settings.connect('changed::' + s, function (k,b) {
            settings_spin.set_value(window._settings.get_int(b));
        });
    });
 ...
 ******************************************************************************/
    ...
    ['animation-time-overview',
     'animation-time-autohide',
        ...
        let settings_spin = builder.get_object("spin_" + s.replace(/-/g, "_"));
        settings_spin.set_value(window._settings.get_double(s));
        settings_spin.connect('value-changed', function (w) {
            settings.set_double(s, w.get_value());
        });
        window._settings.connect('changed::' + s, function (k,b) {
            settings_spin.set_value(window._settings.get_double(b));
        });
    });
...
/* ++++++++++++++++++++++++++++++++++++ Keyboard accelerator +++++ */
    ...
    let model = builder.get_object("store_shortcut_keybind");
    let model_row = model.get_iter_first()[1];
    let binding = window._settings.get_strv('shortcut-keybind')[0],
        binding_key,
        binding_mods;
    if (binding) {
        [binding_key, binding_mods] = Gtk.accelerator_parse(binding);
    ...
    cellrend.connect('accel-edited',
      function (rend, iter, binding_key, binding_mods) {
        ...
        model.set(iterator, [0, 1], [binding_mods, binding_key]);
        window._settings.set_strv('shortcut-keybind', [value]);
      });
    ...
    cellrend.connect('accel-cleared',
      function (rend, iter, binding_key, binding_mods) {
        ...
        model.set(iterator, [0, 1], [0, 0]);
        window._settings.set_strv('shortcut-keybind', []);
      });

      window._settings.connect('changed::shortcut-keybind', function (k, b) {
        let binding = window._settings.get_strv('shortcut-keybind')[0];
        let binding_key = binding_mods = 0;
        if (binding) {
            [binding_key, binding_mods] = Gtk.accelerator_parse(binding);
...
/* ++++++++++++++++++++++++++++++++++++ End: Keyboard accelerator +++++ */
    ...
    ['shortcut-delay',
        ...
        settings_spin.set_value(window._settings.get_double(s));
        settings_spin.connect('value-changed', function (w) {
            window._settings.set_double(s, w.get_value());
        });
        window._settings.connect('changed::' + s, function (k,b) {
            settings_spin.set_value(window._settings.get_double(b));
        });
 ...
 ******************************************************************************/
    ...
    ['enable-intellihide',
     'enable-active-window',
        ...
        window._settings.connect('changed::' + s, function (k,b) {
            settings_onoff.set_active(window._settings.get_boolean(b));
        });
    });
